...
static int s_stop(struct aws_event_loop *event_loop) {
  ...
  //    aws_atomic_compare_exchange_ptr(&epoll_loop->stop_task_ptr, &expected_ptr, &epoll_loop->stop_task);
    ...
    void *val = aws_atomic_load_ptr(&epoll_loop->stop_task_ptr);
    if (val == expected_ptr) {
        aws_atomic_store_ptr(&epoll_loop->stop_task_ptr, &epoll_loop->stop_task);
...
static bool s_is_on_callers_thread(struct aws_event_loop *event_loop) {
    struct epoll_loop *epoll_loop = event_loop->impl_data;
    uint64_t sentinel_epoll_loop_thread_id = aws_atomic_load_int(&epoll_loop->thread_id);
    uint64_t sentinel_aws_thread_current_thread_id = aws_thread_current_thread_id();
...
#if 0 //TODO (remove): use epoll signaling
...
#endif
    assert(aws_linked_list_is_valid_deep(&epoll_loop->task_pre_queue));

    aws_linked_list_swap_contents(&epoll_loop->task_pre_queue, &task_pre_queue);

    assert(aws_linked_list_is_valid_deep(&task_pre_queue));
...
static void s_main_loop(void *args) {
...
#if 0 //TODO (remove): reduce code-to-check
    ...
    while (epoll_loop->should_continue) {
        ...
        if (use_default_timeout) {
            ...
            // TODO: following line causes smack problems
