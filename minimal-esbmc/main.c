...
void aws_task_init(struct aws_task *task, aws_task_fn *fn, void *arg, const char *type_tag) {
    ...
    task->fn = 0; task->arg = 0; task->timestamp = 0; task->node.next = 0; task->node.prev = 0; task->type_tag = 0;
    task->fn = fn;
    task->arg = arg;
...
struct aws_event_loop gLoop;
struct epoll_loop gEpollLoop;

struct aws_event_loop *fake_event_loop_new() {
    // model aws_event_loop_new_default
    struct aws_event_loop *event_loop = &gLoop;//calloc(1, (sizeof(*event_loop)));
    //assume(event_loop);
    struct epoll_loop *epoll_loop = &gEpollLoop;//calloc(1, (sizeof(*epoll_loop)));
    //assume(epoll_loop);
